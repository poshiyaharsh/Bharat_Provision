...
final stockDashboardProductsProvider = FutureProvider<List<Product>>((ref) async {
...
final stockCategoriesProvider = FutureProvider<List<StockCategory>>((ref) async {
  final repo = ref.watch(stockRepositoryProvider);
  final cats = await repo.getCategories();
  return cats.map((c) => StockCategory(id: c.id, name: c.nameGujarati)).toList();
});

final expenseAccountsProvider = FutureProvider<List<ExpenseAccount>>((ref) async {
  final repo = ref.watch(stockRepositoryProvider);
  return repo.getExpenseAccounts();
});

final stockHistoryProvider =
    FutureProvider.family<List<StockLogEntry>, HistoryParams>((ref, params) async {
  final repo = ref.watch(stockRepositoryProvider);
